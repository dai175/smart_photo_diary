...
### 収益化システム（フリーミアムモデル）
- **Basic（無料）**: 月10回のAI生成、基本プロンプト5種類（emotionカテゴリ）
- **Premium（月額/年額）**: 月100回のAI生成、全20種類のプロンプト、高度なフィルタ・分析機能、過去365日の写真アクセス
...
├── widgets/            # ドメイン固有ウィジェット（settings/, timeline/, upgrade/ サブディレクトリ + カレンダー・プロンプト選択・日記カード等）
