...
- [ ] `lib/services/analytics/` ディレクトリのバックアップ作成
- [ ] 依存関係の確認（他のファイルからのimport文を検索）
- [ ] 以下のファイルを削除
  - [ ] `category_popularity_reporter.dart` (515行)
  - [ ] `improvement_suggestion_tracker.dart` (845行)
  - [ ] `prompt_usage_analytics.dart` (619行)
  - [ ] `user_behavior_analyzer.dart` (822行)
- [ ] 削除後のビルド確認
- [ ] テストの実行と成功確認
