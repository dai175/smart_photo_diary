...
- **Release flow**: bump version → push tag `vX.Y.Z` → `release.yml` creates a draft (notes only) → publish the draft → `ios-deploy.yml` builds signed IPA + uploads to TestFlight.
