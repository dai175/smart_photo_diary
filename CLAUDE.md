...
- **Subscription status forcePlan**: `SubscriptionStateService.getCurrentStatus()` applies the debug `FORCE_PLAN` override (`applyForcePlan`). Before persisting, always read via `getRawStatus()` / `_getInitializedRawStatus()` so forced plan/expiry values are never written to Hive
...
- `.github/workflows/release.yml` — on tag `v*`: builds iOS (no codesign) and creates a **draft** GitHub Release
- `.github/workflows/ios-deploy.yml` — on release **published**: signs via fastlane match and uploads to TestFlight
