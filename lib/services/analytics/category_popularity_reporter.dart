...
class CategoryReportSummary {
  ...
  String toString() {
    ...
    buffer.writeln(
      'アクティブカテゴリ数: $activeCategories/${PromptCategory.values.length}',
    );

    if (mostPopularCategory != null) {
      buffer.writeln(
        '最人気カテゴリ: ${mostPopularCategory!.displayName} '
        '(${(mostPopularUsageRate * 100).toStringAsFixed(1)}%)',
      );
    }

    buffer.writeln(
      '平均カテゴリ使用率: ${(averageCategoryUsage * 100).toStringAsFixed(1)}%',
    );
...
class TrendSummary {
  ...
  String toString() {
    ...
    if (fastestGrowingCategory != null && maxGrowthRate > 0) {
      buffer.writeln(
        '最高成長: ${fastestGrowingCategory!.displayName} '
    ...
    if (fastestDecliningCategory != null && maxDeclineRate < 0) {
      buffer.writeln(
        '最大衰退: ${fastestDecliningCategory!.displayName} '
...
class CategoryPopularityReporter {
  ...
  /// サマリーを生成
  static CategoryReportSummary _generateSummary(
    CategoryPopularityAnalysis analysis,
  ) {
    final totalUsage = analysis.categoryUsageCount.values.fold<int>(
      0,
      (sum, count) => sum + count,
    );
  ...
  /// カテゴリ詳細を生成
  static List<CategoryDetail> _generateCategoryDetails(
    CategoryPopularityAnalysis analysis,
  ) {
    ...
    for (int i = 0; i < sortedEntries.length; i++) {
      ...
      final averageUsagePerPrompt =
          analysis.averageUsagePerPrompt[category] ?? 0.0;
  ...
  /// トレンド要約を生成
  static TrendSummary _generateTrendSummary(
  ...
  static String generateQuickSummary(CategoryPopularityAnalysis analysis) {
    ...
    if (analysis.mostPopularCategory != null) {
      ...
      summary +=
          '最も人気は「${popularCategory.displayName}」で${(usageRate * 100).toStringAsFixed(0)}%の使用率です。';
    }
  ...
  static List<CategoryRankingItem> getCategoryRanking({
    ...
    final sortedEntries =
        analysis.categoryUsageCount.entries
