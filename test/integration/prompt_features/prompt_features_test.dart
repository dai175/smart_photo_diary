  ...
  group('Phase 3.1.2: プロンプト機能統合テスト', () {
    ...
    group('3.1.2.2: Basic/Premium分離テスト', () {
      test('Basic/Premium用プロンプトの正確な分離', () async {
        ...
        final premiumPrompts = allPrompts
        ...
        expect(
          premiumCategories.contains(PromptCategory.sensoryEmotion),
          isTrue,
        );
        expect(
          premiumCategories.contains(PromptCategory.emotionGrowth),
          isTrue,
        );
        expect(
          premiumCategories.contains(PromptCategory.emotionConnection),
          isTrue,
        );
        expect(
          premiumCategories.contains(PromptCategory.emotionDiscovery),
          isTrue,
        );
        expect(
          premiumCategories.contains(PromptCategory.emotionFantasy),
          isTrue,
        );
        expect(
          premiumCategories.contains(PromptCategory.emotionHealing),
          isTrue,
        );
        expect(
          premiumCategories.contains(PromptCategory.emotionEnergy),
          isTrue,
        );
      });
      ...
      test('プラン権限による適切なフィルタリング', () async {
        ...
        final emotionDepthPrompts = promptService.getPromptsByCategory(
      ...
      test('無効なプランでのアクセス制限', () async {
        ...
        final expiredPrompts = promptService.getPromptsForPlan(
          isPremium: false,
        );

        // 期限切れの場合はBasicプランと同様の動作をするか確認
        final premiumPrompts = expiredPrompts
            .where((p) => p.isPremiumOnly)
            .toList();
    ...
    group('3.1.2.3: プロンプト検索テスト', () {
      test('キーワード検索の基本動作', () async {
        ...
        for (final prompt in searchResults) {
          final containsKeyword =
              prompt.text.contains('感情') ||
              (prompt.description?.contains('感情') ?? false) ||
              prompt.tags.any((tag) => tag.contains('感情'));
          expect(containsKeyword, isTrue, reason: '検索結果にはキーワードが含まれている必要があります');
        }

        // 複数キーワードの検索
        final multiResults = promptService.searchPrompts(
          '気持ち',
          isPremium: true,
        );
        ...
        if (multiResults.isNotEmpty) {
          for (final prompt in multiResults) {
            final containsAnyKeyword =
                prompt.text.contains('気持ち') ||
      ...
      test('プラン制限下での検索結果フィルタリング', () async {
        ...
        // Premium用キーワードでの検索
        final premiumKeywordResults = promptService.searchPrompts(
          '深掘り',
          isPremium: false,
        );
        ...
        // 返っても少ない結果になる
        expect(
          premiumKeywordResults.isEmpty ||
              premiumKeywordResults.every((p) => !p.isPremiumOnly),
          isTrue,
        );
        ...
        expect(
          ...
          greaterThanOrEqualTo(premiumKeywordResults.length),
        );
      });
      ...
      test('空検索結果の適切な処理', () async {
        ...
        // 空白文字での検索
        final whitespaceResults = promptService.searchPrompts(
          '   ',
          isPremium: true,
        );
    ...
    group('3.1.2.4: カテゴリフィルタテスト', () {
      test('カテゴリ別フィルタリング機能', () async {
        ...
        for (final category in categories) {
          // When: カテゴリでフィルタリング
          final categoryPrompts = promptService.getPromptsByCategory(
            category,
            isPremium: true,
          );
      ...
      test('複数カテゴリでの組み合わせテスト', () async {
        ...
        for (final prompt in allPrompts) {
          categoryStats[prompt.category] =
              (categoryStats[prompt.category] ?? 0) + 1;
        }
        ...
        final accessibleBasicCategories = basicPlanPrompts
            .map((p) => p.category)
            .toSet();

        expect(accessibleBasicCategories.length, equals(1));
        expect(
          accessibleBasicCategories.contains(PromptCategory.emotion),
          isTrue,
        );
        ...
        final accessiblePremiumCategories = premiumPlanPrompts
            .map((p) => p.category)
            .toSet();
