...
void main() {
  group(
    'Phase 2-2: Past Photos機能統合テスト - プラン制限と365日範囲日付計算',
    () {
      ...
      void setupLoggingServiceMock(MockLoggingService mock) {
        when(() => mock.info(any(), context: any(named: 'context'), data: any(named: 'data')))
            .thenReturn(null);
        when(() => mock.debug(any(), context: any(named: 'context'), data: any(named: 'data')))
            .thenReturn(null);
        when(() => mock.warning(any(), context: any(named: 'context'), data: any(named: 'data')))
            .thenReturn(null);
        when(() => mock.error(
              any(),
      ...
      void setupPhotoServiceMock(MockPhotoServiceInterface mock, List<MockAssetEntity> photos) {
        when(() => mock.getPhotosEfficientResult(
      ...
      void setupSuccessfulPhotoServiceMock(MockPhotoServiceInterface mock, List<MockAssetEntity> photos) {
        when(() => mock.getPhotosEfficientResult(
              startDate: any(named: 'startDate'),
              endDate: any(named: 'endDate'),
              limit: any(named: 'limit'),
      ...
      List<MockAssetEntity> createMockPhotosWithDates() {
        ...
        for (int i = 1; i <= 30; i++) {
          ...
          when(() => photo.createDateTime).thenReturn(photoDate.add(Duration(hours: 12))); // 12時に撮影
          when(() => photo.modifiedDateTime).thenReturn(photoDate.add(Duration(hours: 12)));
      ...
      List<MockAssetEntity> createMockPhotosForDateRangeTest() {
        ...
        when(() => photo1.createDateTime).thenReturn(oneDayAgo.add(const Duration(hours: 10)));
      ...
      group('Group 1: プラン制限統合テスト - Basic vs Premium実動作確認', () {
        test('Basic Plan - 1日前までのアクセス制限確認', () {
          ...
          // Act
          final accessibleDate = photoAccessControlService.getAccessibleDateForPlan(basicPlan);
          ...
          // 1日前の写真はアクセス可能
          final oneDayAgo = today.subtract(const Duration(days: 1));
          expect(photoAccessControlService.isPhotoAccessible(oneDayAgo, basicPlan), isTrue);
          
          // 2日前の写真はアクセス不可
          final twoDaysAgo = today.subtract(const Duration(days: 2));
          expect(photoAccessControlService.isPhotoAccessible(twoDaysAgo, basicPlan), isFalse);
        });
        ...
        test('Premium Plan - 365日前までのアクセス制限確認', () {
          ...
          expect(accessibleDate, equals(today.subtract(const Duration(days: 365))));
          
          // 365日前の写真はアクセス可能
          final day365Ago = today.subtract(const Duration(days: 365));
          expect(photoAccessControlService.isPhotoAccessible(day365Ago, premiumPlan), isTrue);
          
          // 366日前の写真はアクセス不可
          final day366Ago = today.subtract(const Duration(days: 366));
          expect(photoAccessControlService.isPhotoAccessible(day366Ago, premiumPlan), isFalse);
        });
        ...
        test('プラン別アクセス可能日数の差異確認', () {
          ...
          // 同じ日付での判定結果比較
          final now = DateTime.now();
          final testDate = DateTime(now.year, now.month, now.day).subtract(const Duration(days: 30));
          
          expect(photoAccessControlService.isPhotoAccessible(testDate, basicPlan), isFalse);
          expect(photoAccessControlService.isPhotoAccessible(testDate, premiumMonthlyPlan), isTrue);
          expect(photoAccessControlService.isPhotoAccessible(testDate, premiumYearlyPlan), isTrue);
        });
      ...
      group('Group 2: 365日範囲日付計算ロジック精密テスト', () {
        test('タイムゾーン対応 - 日付のみ比較（時刻は考慮しない）', () {
          ...
          final midnightPhoto = DateTime(now.year, now.month, now.day, 23, 59, 59);
          
          // Act & Assert - 今日の写真は時刻に関係なくアクセス可能（実際は除外されるが判定はtrue）
          expect(photoAccessControlService.isPhotoAccessible(morningPhoto, premiumPlan), isTrue);
          expect(photoAccessControlService.isPhotoAccessible(eveningPhoto, premiumPlan), isTrue);
          expect(photoAccessControlService.isPhotoAccessible(midnightPhoto, premiumPlan), isTrue);
          
          // 365日前の異なる時刻での写真
          final day365Morning = today.subtract(const Duration(days: 365)).add(const Duration(hours: 8));
          final day365Evening = today.subtract(const Duration(days: 365)).add(const Duration(hours: 20));
          
          expect(photoAccessControlService.isPhotoAccessible(day365Morning, premiumPlan), isTrue);
          expect(photoAccessControlService.isPhotoAccessible(day365Evening, premiumPlan), isTrue);
        });
        ...
        test('境界値テスト - 365日前後の厳密な判定', () {
          ...
          expect(photoAccessControlService.isPhotoAccessible(day364Ago, premiumPlan), isTrue);
          expect(photoAccessControlService.isPhotoAccessible(day365Ago, premiumPlan), isTrue);
          expect(photoAccessControlService.isPhotoAccessible(day366Ago, premiumPlan), isFalse);
        });
        ...
        test('うるう年対応 - 2月29日を含む年の日付計算', () {
          ...
          // うるう年のテストケース (2024年2月29日)
          final leapYearDate = DateTime(2024, 2, 29);
          final accessibleDate = leapYearDate.subtract(Duration(days: premiumPlan.pastPhotoAccessDays));
          
          // Act
          final result = photoAccessControlService.isPhotoAccessible(accessibleDate, premiumPlan);
          
          // Assert
          expect(result, isTrue);
          
          // 366日前はアクセス不可（うるう年でも365日制限）
          final day366Before = leapYearDate.subtract(const Duration(days: 366));
          expect(photoAccessControlService.isPhotoAccessible(day366Before, premiumPlan), isFalse);
        });
        ...
        test('月末日境界での日付計算確認', () {
          ...
          final accessibleFromJan31 = jan31.subtract(const Duration(days: 365));
          
          // 2月28日から365日前を計算（非うるう年）
          final feb28_2023 = DateTime(2023, 2, 28);
          final accessibleFromFeb28 = feb28_2023.subtract(const Duration(days: 365));
          
          // Act & Assert
          expect(photoAccessControlService.isPhotoAccessible(accessibleFromJan31, premiumPlan), isTrue);
          expect(photoAccessControlService.isPhotoAccessible(accessibleFromFeb28, premiumPlan), isTrue);
          
          // より古い日付はアクセス不可
          final oneDayEarlier1 = accessibleFromJan31.subtract(const Duration(days: 1));
          final oneDayEarlier2 = accessibleFromFeb28.subtract(const Duration(days: 1));
          expect(photoAccessControlService.isPhotoAccessible(oneDayEarlier1, premiumPlan), isFalse);
          expect(photoAccessControlService.isPhotoAccessible(oneDayEarlier2, premiumPlan), isFalse);
        });
        ...
        test('getAccessibleDateForPlan() 精密計算テスト', () {
          ...
          // Act
          final basicAccessible = photoAccessControlService.getAccessibleDateForPlan(basicPlan);
          final premiumAccessible = photoAccessControlService.getAccessibleDateForPlan(premiumPlan);
          
          // Assert
          expect(basicAccessible, equals(today.subtract(const Duration(days: 1))));
          expect(premiumAccessible, equals(today.subtract(const Duration(days: 365))));
          
          // 日付差の確認
          final dayDifference = basicAccessible.difference(premiumAccessible).inDays;
      ...
      group('Group 5: 日付フィルタリング統合テスト', () {
        ...
        test('取得された写真の日付がプラン制限範囲内であることの確認', () async {
          ...
          for (final photo in photos) {
            ...
            expect(
              photoDate.isAfter(accessibleDate) || photoDate.isAtSameMomentAs(accessibleDate),
              isTrue,
      ...
      group('Group 6: エラーハンドリング統合テスト', () {
        test('PhotoAccessException処理確認', () async {
          ...
          when(() => mockPhotoService.getPhotosEfficientResult(
                ...
                endDate: any(named: 'endDate'),
                limit: any(named: 'limit'),
              )).thenAnswer((_) async => Failure(PhotoAccessException('写真アクセス権限が拒否されました')));
