  ...
  group('Phase 1.6.2: In-App Purchase機能統合テスト', () {
    ...
    group('Phase 1.6.2.1: 商品情報取得実装テスト', () {
      ...
      test('getProducts()が正常にResult<T>パターンで応答する', () async {
        ...
        if (result.isFailure) {
          // テスト環境でのエラーは期待される動作
          expect(result.error.toString(), contains('In-App Purchase not available'));
        } else {
          // 万が一成功した場合（実デバイステスト等）
    ...
    group('Phase 1.6.2.2: 購入フロー実装テスト', () {
      ...
      test('purchasePlan()でBasicプラン購入時にエラーが返される', () async {
        // Basicプランは購入対象外
        final result = await subscriptionService.purchasePlan(SubscriptionPlan.basic);
      ...
      test('purchasePlan()でPremiumプラン指定時の動作確認', () async {
        // Premium月額プランの購入試行
        final result = await subscriptionService.purchasePlan(SubscriptionPlan.premiumMonthly);
      ...
      test('購入中の重複購入防止機能確認', () async {
        ...
        final monthlyResult = await subscriptionService.purchasePlan(SubscriptionPlan.premiumMonthly);
        final yearlyResult = await subscriptionService.purchasePlan(SubscriptionPlan.premiumYearly);
    ...
    group('Phase 1.6.2.3: 購入状態監視実装テスト', () {
      ...
      test('purchaseStreamが利用可能である', () async {
        ...
        expect(subscriptionService.purchaseStream, isA<Stream<PurchaseResult>>());
        
        // ストリームが購読可能であることを確認
        var streamReceived = false;
        late StreamSubscription subscription;
        
        subscription = subscriptionService.purchaseStream.timeout(
      ...
      test('statusStreamが正常に動作する', () async {
        ...
        expect(subscriptionService.statusStream, isA<Stream<SubscriptionStatus>>());
        
        // ストリームからデータを取得できることを確認
        var statusReceived = false;
        late StreamSubscription subscription;
        
        subscription = subscriptionService.statusStream.timeout(
    ...
    group('Phase 1.6.2.4: 復元機能実装テスト', () {
      ...
      test('validatePurchase()の基本動作確認', () async {
        ...
        final result = await subscriptionService.validatePurchase(testTransactionId);
      ...
      test('changePlan()が未実装エラーを返すことを確認', () async {
        final result = await subscriptionService.changePlan(SubscriptionPlan.premiumYearly);
    ...
    group('In-App Purchase統合確認テスト', () {
      ...
      test('全購入関連メソッドがResult<T>パターンを使用している', () async {
        ...
        final purchaseResult = await subscriptionService.purchasePlan(SubscriptionPlan.premiumMonthly);
        ...
        final validateResult = await subscriptionService.validatePurchase('test');
      ...
      test('エラー処理の一貫性確認', () async {
        ...
        // 無効な入力でのエラー
        final basicPurchaseResult = await subscriptionService.purchasePlan(SubscriptionPlan.basic);
