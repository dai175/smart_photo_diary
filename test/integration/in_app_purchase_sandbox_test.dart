  ...
  group('Phase 1.6.1.4: In-App Purchase Sandbox Test Preparation', () {
    ...
    group('Product Configuration Tests', () {
      ...
      test('価格設定が正しく設定されている', () {
        ...
        // 年額割引計算が正しい
        expect(InAppPurchaseConfig.yearlyDiscountAmount, equals(800)); // 300*12 - 2800
        expect(InAppPurchaseConfig.yearlyDiscountPercentage, closeTo(22.22, 0.1));
      });
      ...
      test('プランと商品IDのマッピングが正しい', () {
        ...
        // 商品IDからプランを取得
        expect(
          InAppPurchaseConfig.getSubscriptionPlan('smart_photo_diary_premium_monthly'),
          equals(SubscriptionPlan.premiumMonthly),
        );
        expect(
          InAppPurchaseConfig.getSubscriptionPlan('smart_photo_diary_premium_yearly'),
      ...
      test('商品説明文が設定されている', () {
        // 表示名が設定されている
        expect(InAppPurchaseConfig.getDisplayName(SubscriptionPlan.premiumMonthly), isNotEmpty);
        expect(InAppPurchaseConfig.getDisplayName(SubscriptionPlan.premiumYearly), isNotEmpty);
        expect(InAppPurchaseConfig.getDisplayName(SubscriptionPlan.basic), isNotEmpty);
        
        // 説明文が設定されている
        expect(InAppPurchaseConfig.getDescription(SubscriptionPlan.premiumMonthly), isNotEmpty);
        expect(InAppPurchaseConfig.getDescription(SubscriptionPlan.premiumYearly), isNotEmpty);
        expect(InAppPurchaseConfig.getDescription(SubscriptionPlan.basic), isNotEmpty);
    ...
    group('Regional Pricing Tests', () {
      ...
      test('価格フォーマット関数が正しく動作する', () {
        ...
        expect(InAppPurchaseConfig.formatPrice(28.99, 'USD'), equals('\$28.99'));
      });
      ...
      test('地域別価格が適切に設定されている', () {
        for (final region in InAppPurchaseConfig.regionalPricing.values) {
          ...
          final discountPercentage = ((expectedYearlyPrice - yearlyPrice) / expectedYearlyPrice) * 100;
    ...
    group('Free Trial Configuration Tests', () {
      ...
      test('無料トライアル対象商品が正しく設定されている', () {
        ...
        // 対象商品チェック
        expect(
          InAppPurchaseConfig.isEligibleForFreeTrial('smart_photo_diary_premium_monthly'),
          isTrue,
        );
        expect(
          InAppPurchaseConfig.isEligibleForFreeTrial('smart_photo_diary_premium_yearly'),
          isTrue,
        );
        expect(
    ...
    group('Test Environment Configuration Tests', () {
      test('テストアカウントが設定されている', () {
        ...
        // テストアカウントのフォーマット確認
        expect(InAppPurchaseConfig.testAccounts['ios'], contains('@smartphotodiary.app'));
        expect(InAppPurchaseConfig.testAccounts['android'], contains('@smartphotodiary.app'));
      });
      ...
      test('プラットフォーム設定が定義されている', () {
        ...
        expect(InAppPurchaseConfig.androidConfig['licenseTestingEnabled'], isTrue);
      });
    ...
    group('Configuration Validation Tests', () {
      test('設定の妥当性が検証される', () {
        ...
        expect(errors, isEmpty, reason: 'Configuration validation errors: ${errors.join(', ')}');
      ...
      test('商品IDの妥当性チェックが動作する', () {
        // 有効な商品ID
        expect(InAppPurchaseConfig.isValidProductId('smart_photo_diary_premium_monthly'), isTrue);
        expect(InAppPurchaseConfig.isValidProductId('smart_photo_diary_premium_yearly'), isTrue);
        
        // 無効な商品ID
        expect(InAppPurchaseConfig.isValidProductId('invalid_product_id'), isFalse);
        expect(InAppPurchaseConfig.isValidProductId(''), isFalse);
      });
      
      test('購入可能プランの判定が正しい', () {
        // 購入可能プラン
        expect(InAppPurchaseConfig.isPurchasable(SubscriptionPlan.premiumMonthly), isTrue);
        expect(InAppPurchaseConfig.isPurchasable(SubscriptionPlan.premiumYearly), isTrue);
        
        // 購入不可プラン
        expect(InAppPurchaseConfig.isPurchasable(SubscriptionPlan.basic), isFalse);
      });
    });
    ...
    group('Integration Confirmation Tests', () {
      ...
      test('全体的な設定の一貫性確認', () {
        ...
        for (final productId in InAppPurchaseConfig.allProductIds) {
          ...
          expect(() => InAppPurchaseConfig.getSubscriptionPlan(productId), returnsNormally);
        }
