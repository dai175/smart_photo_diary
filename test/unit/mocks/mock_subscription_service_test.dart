  ...
  group('MockSubscriptionService - Phase 1.5.1テスト', () {
    ...
    group('アクセス権限チェック', () {
      ...
      test('Premiumプランでのアクセス権限が正しい', () async {
        mockService.setCurrentPlan(SubscriptionPlan.premiumYearly, isActive: true);
    ...
    group('購入・復元機能モック', () {
      ...
      test('利用可能な商品を取得できる', () async {
        ...
        expect(products.any((p) => p.plan == SubscriptionPlan.premiumMonthly), isTrue);
        expect(products.any((p) => p.plan == SubscriptionPlan.premiumYearly), isTrue);
      });

      test('プラン購入をシミュレートできる', () async {
        final purchaseResult = await mockService.purchasePlan(SubscriptionPlan.premiumYearly);
        
        expect(purchaseResult.isSuccess, isTrue);
        expect(purchaseResult.value.status, equals(PurchaseStatus.purchased));
        expect(purchaseResult.value.plan, equals(SubscriptionPlan.premiumYearly));
    ...
    group('エラーシミュレーション', () {
      ...
      test('購入失敗をシミュレートできる', () async {
        ...
        final result = await mockService.purchasePlan(SubscriptionPlan.premiumYearly);
